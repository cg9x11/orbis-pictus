...
- **Default providers** (all swappable by env). DECIDED 2026-08: the user runs an **Anthropic-compatible** LLM proxy (Anthropic Messages API, NOT OpenAI chat-completions) at `http://localhost:20128/v1` (key in `.env` as `LLM_API_KEY`, never hardcoded) which is the default for ALL text/vision LLM work:
  ...
  - Image: DECIDED 2026-08-06 — **BytePlus Ark (ModelArk)**, replacing fal.ai (fal balance exhausted; Ark is cheaper and the user activated "Free Credit Only" mode, i.e. requests are free until per-model quota runs out, then the API errors instead of charging). Provider `ark` using Ark's images API (`POST {ARK_BASE_URL}/api/v3/images/generations`, key `ARK_API_KEY`, endpoint like `https://ark.ap-southeast.bytepluses.com`). Primary model: **`seedream-4-5-251128`** (Seedream 4.5 — strong text rendering, ~$0.03/image when paid; native image-input editing covers our `edit`/tap modes without SeedEdit). Fallback pool: **`seedream-4-0-250828`** (separate free quota — auto-fallback on quota-exhausted errors). Model IDs confirmed by the user from the Ark console 2026-08-06.
    ...
    **Residual risk found during launch-asset capture (2026-08-06, not fully closed):** across 8 real generations run to validate the fix and produce showcase pages, 7/8 had zero numeral badges (confirming the fix holds in the large majority of cases), but one "How a Volcano Erupts" regeneration spontaneously drew circled numeral badges (①②③⑤⑥⑦, skipping ④) next to `"Stage one — ...", "Stage two — ..."` labels even though the prompt explicitly said "no numeral badges" — the same duplicate/skip pattern as the original bug. The one common thread: this was the only affected generation whose labels leaned hardest on the word-ordinal convention this same fix recommends ("Stage one", "Stage two", ... "Final stage A/B") combined with a drawn arrow path connecting them in order — i.e. the fix for preserving genuine sequence may itself weakly cue the model to also draw matching digit badges, on top of already not being reliable at digit-as-marker rendering. Not mitigated further this session (would need a larger sample to isolate); worth watching, and worth trying alternatives to numeric word-ordinals (e.g. "First — ...", "Then — ...", "Finally — ...") if it recurs.
    **Second, separate residual issue found the same session:** independent of numeral badges, dense pages (7-8 callouts) still sometimes merge two adjacent callouts' text into one garbled plaque, or drop a plaque's label entirely while its leader line and drawn subject still appear correctly — seen on "How a Mechanical Wristwatch Works" (typo + a dropped label), "How a Lighthouse Fresnel Lens Works" first attempt (a duplicated, mismatched "Lower Prism" plaque), "How a Volcano Erupts" first attempt (a duplicated "Final stage B" plaque), and "Coral Reef Ecosystem" both attempts (one merged-label plaque each). This is the same content-duplication failure mode `page-author.md`'s callout-pairing rule was written to prevent, but pairing alone doesn't fully eliminate it at high callout counts — it appears to be a general Seedream density limit, distinct from the numeral-badge bug, and out of scope for this session's fix. Lower callout counts (4-6) were visibly more reliable in this sample than 7-8; worth considering a tighter default range if this keeps recurring.
    **Confirmed and fixed post-launch (2026-08-06), found by inspecting the demo's hero page at full resolution:** the >6-callout pairing-collapse risk flagged just above was real and reproduced on the shipped demo. Root cause was a prompt-contract bug, not a pure model limitation: `house-style.md`'s layout section already capped the scene at "five or six labelled elements", but `page-author.md` told the author LLM to "Include 4 to 8" — a direct contradiction, and the author prompt is the only one of the two the LLM actually reads when deciding content. The Roadway Deck child page authored 7 callouts and Seedream shuffled the label/description pairs one slot down the list while descriptions stayed put: "Stiffening Truss" rendered with a different callout's description, "Suspender Rope Connection" both misspelled ("Suspening Truss") and given the wrong text, "East Sidewalk" labelled "Six Traffic Lanes", "West Sidewalk" labelled "East Sidewalk". The parent Golden Gate page (also 7 callouts) showed a milder version of the same decay. Separately, the same hero page had a free-invented subtitle line under the title card ("The traffic-carrying surface of...") — `house-style.md` already forbade this and stated the italicized general rule that any such layout constraint must ship with matching instructions in `page-author.md`/`edit-author.md`, but that mirroring had never been done for either the callout cap or the subtitle ban. Fixed by lowering `page-author.md`'s cap to 4–6 (matching house-style, with the pairing-collapse evidence inline as the stated reason) and adding an explicit title-card-is-title-only rule to both `page-author.md` and `edit-author.md`. General lesson: `house-style.md` is appended to the *image* prompt, but the page-author LLM that decides content only ever sees `page-author.md`/`edit-author.md` — any layout constraint stated only in `house-style.md` is invisible to the LLM that needs to obey it and must be mirrored explicitly, not assumed to propagate.
    **Regeneration result, same session — fix confirmed partial, not complete:** re-authoring the Roadway Deck page under the corrected prompt (6 callouts, explicit no-subtitle rule) and regenerating the image produced a *different* failure in the same family rather than a clean page: the subtitle was gone and no label drifted onto the wrong subject, but the image still duplicated two plaques — "Movable Median Barrier" rendered twice, its second copy's description overwritten with a fragment of the "Stiffening Truss" description ("steel barrier shifted by a zipper truck to stabilizes the deck") and no leader line of its own, and "East Sidewalk" rendered twice verbatim, its second copy's leader line landing on open roadway rather than a sidewalk. Accepted as shipped (2026-08-06) rather than spending further Free-Credit-Only image/video quota chasing a clean generation — net strictly better than the pre-fix version (no subtitle, no cross-callout label drift) even though duplication persisted. Confirms the "Second, separate residual issue" paragraph above: lowering the callout ceiling reduces the *drift* variant of the pairing-collapse bug but does not eliminate plain *duplication* as a failure mode, even at 6 callouts on a single retry — this reads as a genuine Seedream density/attention limit rather than something further prompt wording alone fixes, and is the next thing to revisit if a more reliable image model becomes available.
...
**Transition morphs — GO/NO-GO gate result: GO (verified 2026-08-06).** One live morph generated directly against Ark (script `apps/server/src/scripts/gateTestMorph.ts`, since deleted — reproducible from this note) on the Golden Gate Bridge → Roadway Deck pair already in the dev DB (the tapped subject appears in both images, per §1.5's own explanation of why the original's morphs read well). First frame = parent's `landscape.jpg`, last frame = child's `landscape.jpg`, `seedance-1-0-pro-250528`, 480p/5s/16:9, camera-fixed flag set. Judged frame-by-frame (10 frames extracted at 2fps):
  - **Bug found and fixed before the clip would even generate**: Ark rejects multi-image (first+last-frame) content with `The parameter content specified in the request is not valid: role must be specified for image contents` unless each `image_url` part also carries `role: "first_frame"` / `role: "last_frame"`. The single-image idle-loop path has no such requirement (already verified working without a role) — `providers/video/ark.ts` now only adds `role` when `lastFrameDataUrl` is present, leaving the idle-loop request shape untouched. This rejection happened at request-validation time, before any generation was billed, so it did not consume any of the session's 3-generation budget.
  - **Subject anchoring**: reads as a smooth camera zoom from the full-bridge establishing shot into a close-up on the roadway deck, not a literal fixed frame despite `--camerafixed true` — but the tapped subject stays centered and legible throughout, and the zoom itself reads as an intentional "drill into what you tapped" move rather than drift.
  - **Mid-clip text**: warps through legible-adjacent states ("Golden Gate Bridge" → "Gol dın Gate Bridge" → "Roadway Deck") — reads as "painting in", not broken garbage. Consistent with the already-documented Seedance small-text-instability-under-motion risk above, not a new failure mode.
  - **Landing accuracy**: last frame is close to the real child image but not pixel-exact (missing one label plaque, "East Sidewalk: reserved for bicycles") — expected and non-blocking, since the client design (§3 Phase 5) already swaps to the node's actual stored image once the clip finishes rather than trusting the generated last frame as the resting state.
  - Budget spent on the gate: 1 of 3 generations. Saved clip and extracted frames were in `morph-gate-test/` (repo root, gitignored, deleted after review — not shipped).
...
**Bug found & fixed pre-launch (2026-08-06):** the `/images/*` static route computed its root via `path.dirname(imagesDir)`, relying on the request URL's own `/images/` prefix to line up with the last path segment of `IMAGES_DIR`. This only worked because the default `IMAGES_DIR` (`./data/images`) happens to be named `images` — any other folder name (e.g. `./data/photos`) made every image request silently miss and fall through to the SPA catch-all route, which returns a 200 with `index.html` instead of a 404 or the actual image. A prime first-clone trap since nothing errors, images just quietly become tiny broken-image icons. Fixed by pointing `root` at `imagesDir` directly and stripping the `/images` URL prefix explicitly via `rewriteRequestPath`, so the route no longer depends on the two happening to match. Verified against both the default dir name and a renamed one (`./data/photos`) — both serve the real file with the right content-type.
