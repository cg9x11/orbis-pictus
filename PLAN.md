...
- **Default providers** (mirroring the original, all swappable by env):
  - Prompt author + tap understanding: `gemini-2.x-flash` (fast, cheap, good vision) — adapter interface so Claude/GPT work too.
  - Image: fal.ai `nano-banana` (Gemini image) for final; `fal-ai/flux/schnell` as the cheap **draft** tier. fal supports webhooks/streaming for progressive previews.
  - Web search: Tavily (free tier) behind a `SearchProvider` interface; off by default if no key.
...
### Phase 5 — Live video stream (experimental, separate track)
- Self-host LTX-2 (Lightricks LTX-Video) streaming on Modal following §1.5 protocol (`start`/`set_target_image`/`stop`, fMP4 out, MSE playback, `anchor_loop` looping). This is a large GPU project; scope it only after Phases 0–3 ship. MVP alternative: pre-generate a short loop per page via fal video models and crossfade.
