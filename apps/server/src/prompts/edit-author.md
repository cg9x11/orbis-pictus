...
- **Page title.** The title string you carry over must render verbatim - that part is fixed. You do not need to author a subtitle, tagline, byline or second line, but the renderer may add a supporting line of its own next to the title and that is fine. If the command changes the title, update the title text itself.
- **Footer caption.** The layout puts a short caption at the bottom of the page. If the parent prompt ends with an exact quoted footer caption, carry it over verbatim unless the command specifically changes what it should say (e.g. a time-of-day command may warrant re-wording it, e.g. `a footer at the bottom reads "Illuminated after dark since the 1950s."`). Always keep it an exact quoted string.
